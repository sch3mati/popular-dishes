...
const dishCount = 10000000;
const reviewsPerDish = 3;
const dataGen = async () => {
  const reviewsGenerator = async () => {
    writer.pipe(fs.createWriteStream('./database/data-storage/reviews4_records.csv'));
    for (let whichDish = 7500001; whichDish <= dishCount; whichDish += 1) {
