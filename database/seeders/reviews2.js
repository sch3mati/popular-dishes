...
const dishCount = 9000000;
const reviewsPerDish = 3;
const dataGen = async () => {
  const reviewsGenerator = async () => {
    writer.pipe(fs.createWriteStream('./database/data-storage/reviews2_records.csv'));
    for (let whichDish = 4500001; whichDish <= dishCount; whichDish += 1) {
