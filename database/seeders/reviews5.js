...
const dishCount = 12500000;
const reviewsPerDish = 3;
const dataGen = async () => {
  const reviewsGenerator = async () => {
    writer.pipe(fs.createWriteStream('./database/data-storage/reviews5_records.csv'));
    for (let whichDish = 10000001; whichDish <= dishCount; whichDish += 1) {
